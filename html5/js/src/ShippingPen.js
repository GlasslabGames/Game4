...
GlassLab.ShippingPen.prototype.calculateResult = function(targetNumCreatures, targetTotalFood) {
    ...
    var incorrectNumCreatures = (targetNumCreatures != this.numCreatures);
    ...
    for (var i = 0; i < info.desiredFood.length; i++) { // for each food the creature wants to eat
        var type = info.desiredFood[i].type;
        var index = this.foodTypes.indexOf(type);
        if (index == -1) console.error(type,"wasn't found in ShippingPen.foodTypes even though we should have checked for wrong food type already!");
        else {
